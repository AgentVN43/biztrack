...
const processDateFilters = (query) => {
  ...
  if (query.year) {
    ...
    if (!isNaN(year)) {
      effectiveStartDate = format(startOfYear(new Date(year, 0, 1)), 'yyyy-MM-dd');
  ...
  if (query.month && query.year) {
    ...
    if (!isNaN(year) && !isNaN(month) && month >= 0 && month <= 11) {
      effectiveStartDate = format(startOfMonth(new Date(year, month, 1)), 'yyyy-MM-dd');
      effectiveEndDate = format(endOfMonth(new Date(year, month, 1)), 'yyyy-MM-dd');
    }
  }
...
module.exports = { processDateFilters };